...
print("-------------------")
print()
with open(pwd+"/../../data/INIT_IDX_DONE_pred"+str(PRED_HORIZON_STEP)+"_cntl8_mem"+str(MEMBER)+"_inf"+"{:.2f}".format(MEM_INF_DICT[MEMBER])+"_u3_sp15x.csv", 'w', encoding="utf-8") as f :
    csv.writer(f)
for idx in INIT_IDX_DONE_SET :
    with open(pwd+"/../../data/INIT_IDX_DONE_pred"+str(PRED_HORIZON_STEP)+"_cntl8_mem"+str(MEMBER)+"_inf"+"{:.2f}".format(MEM_INF_DICT[MEMBER])+"_u3_sp15x.csv", 'a', encoding="utf-8", newline="") as f :
