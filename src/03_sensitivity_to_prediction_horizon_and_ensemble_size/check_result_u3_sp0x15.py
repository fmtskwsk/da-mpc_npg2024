...
for p in PRED_HORIZON_STEP_LIST :
    for i, m in enumerate(MEM_LIST) :
        ...
        print("-------------------")
        print()
        with open(pwd+"/../../data/INIT_IDX_DONE_pred"+str(p)+"_cntl8_mem"+str(m)+"_inf"+"{:.2f}".format(MEM_INF_DICT[m])+"_u3_sp0x15.csv", 'w', encoding="utf-8") as f :
            csv.writer(f)
        for idx in INIT_IDX_DONE_SET :
            with open(pwd+"/../../data/INIT_IDX_DONE_pred"+str(p)+"_cntl8_mem"+str(m)+"_inf"+"{:.2f}".format(MEM_INF_DICT[m])+"_u3_sp0x15.csv", 'a', encoding="utf-8", newline="") as f :
