...
class ModelPredictiveControl(DataAssimilation) :
    def __init__(self, PRED_HORIZON_STEP=20, CNTL_HORIZON_STEP=8, MPC_INTERVAL=8, MEMBER_NUM=50, INFLATION=1.04, \
        ...
        U_NORM_MAX=40, PENALTY_PARAM_U=1e3) :
        ...
        self.PENALTY_PARAM_U = PENALTY_PARAM_U
    ...
    def initialize(self) : 
        INIT_IDX_ALL_LIST = []
        INIT_IDX_DONE_LIST = []
        if not os.path.exists(pwd+"/../../data/INIT_IDX_DONE_umax"+str(system.U_NORM_MAX)+"_penalty"+str(system.PENALTY_PARAM_U)+"_u3_umax_sp0x15.csv") :
            with open(pwd+"/../../data/INIT_IDX_DONE_umax"+str(system.U_NORM_MAX)+"_penalty"+str(system.PENALTY_PARAM_U)+"_u3_umax_sp0x15.csv", 'w', encoding="utf-8") as f :
                csv.writer(f)
        with open(pwd+"/../../data/INIT_IDX_ALL_sp0x15.csv", 'r', encoding="utf-8") as f :
            CSV_DATA = list(csv.reader(f, delimiter=','))
            INIT_IDX_ALL_LIST = [int(data[0]) for data in CSV_DATA]
        with open(pwd+"/../../data/INIT_IDX_DONE_umax"+str(system.U_NORM_MAX)+"_penalty"+str(system.PENALTY_PARAM_U)+"_u3_umax_sp0x15.csv", 'r', encoding="utf-8") as f :
            CSV_DATA = list(csv.reader(f, delimiter=','))
        ...
        with open(pwd+"/../../data/INIT_IDX_DONE_umax"+str(system.U_NORM_MAX)+"_penalty"+str(system.PENALTY_PARAM_U)+"_u3_umax_sp0x15.csv", 'a', encoding="utf-8", newline="") as f :
    ...
    def calculate_zeta(self, mu, x_init) :
        ...
        for k in range(self.CNTL_HORIZON_STEP) :
            u_norm = LA.norm(u[k, :])
            if u_norm != 0. :
                zeta[k*self.UNKNOWN_DIM+0] = u[k, 0] + l[k+1, 0] + (u[k, 0] / u_norm) * self.PENALTY_PARAM_U * max((u_norm - self.U_NORM_MAX), 0)
                zeta[k*self.UNKNOWN_DIM+1] = u[k, 1] + l[k+1, 1] + (u[k, 1] / u_norm) * self.PENALTY_PARAM_U * max((u_norm - self.U_NORM_MAX), 0)
                zeta[k*self.UNKNOWN_DIM+2] = u[k, 2] + l[k+1, 2] + (u[k, 2] / u_norm) * self.PENALTY_PARAM_U * max((u_norm - self.U_NORM_MAX), 0)
            else : 
...
def get_args() : 
    ...
    parser.add_argument("u_norm_max", type=int)
    parser.add_argument("penalty_param_u", type=int)
...
for i in range(experiment_num) :
    system = ModelPredictiveControl(U_NORM_MAX=args.u_norm_max, PENALTY_PARAM_U=args.penalty_param_u)
    system.main()
    file_name = pwd+"/../../data/umax"+str(system.U_NORM_MAX)+"_penalty"+str(system.PENALTY_PARAM_U)+"_u3_umax_sp0x15.csv"
