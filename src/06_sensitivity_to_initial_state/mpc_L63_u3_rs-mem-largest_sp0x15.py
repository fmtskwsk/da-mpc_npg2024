...
class ModelPredictiveControl(DataAssimilation) :
    ...
    def initialize(self) : 
        INIT_IDX_ALL_LIST = []
        INIT_IDX_DONE_LIST = []
        if not os.path.exists(pwd+"/../../data/INIT_IDX_DONE_pred"+str(self.PRED_HORIZON_STEP)+"_cntl"+str(self.CNTL_HORIZON_STEP)+"_mem"+str(self.MEMBER_NUM)+"_inf"+"{:.2f}".format(self.INFLATION)+"_u3_rs-mem-largest_sp0x15.csv") :
            with open(pwd+"/../../data/INIT_IDX_DONE_pred"+str(self.PRED_HORIZON_STEP)+"_cntl"+str(self.CNTL_HORIZON_STEP)+"_mem"+str(self.MEMBER_NUM)+"_inf"+"{:.2f}".format(self.INFLATION)+"_u3_rs-mem-largest_sp0x15.csv", 'w', encoding="utf-8") as f :
                csv.writer(f)
        with open(pwd+"/../../data/INIT_IDX_ALL_sp0x15.csv", 'r', encoding="utf-8") as f :
            CSV_DATA = list(csv.reader(f, delimiter=','))
            INIT_IDX_ALL_LIST = [int(data[0]) for data in CSV_DATA]
        with open(pwd+"/../../data/INIT_IDX_DONE_pred"+str(self.PRED_HORIZON_STEP)+"_cntl"+str(self.CNTL_HORIZON_STEP)+"_mem"+str(self.MEMBER_NUM)+"_inf"+"{:.2f}".format(self.INFLATION)+"_u3_rs-mem-largest_sp0x15.csv", 'r', encoding="utf-8") as f :
            CSV_DATA = list(csv.reader(f, delimiter=','))
        ...
        with open(pwd+"/../../data/INIT_IDX_DONE_pred"+str(self.PRED_HORIZON_STEP)+"_cntl"+str(self.CNTL_HORIZON_STEP)+"_mem"+str(self.MEMBER_NUM)+"_inf"+"{:.2f}".format(self.INFLATION)+"_u3_rs-mem-largest_sp0x15.csv", 'a', encoding="utf-8", newline="") as f :
...
for i in range(experiment_num) :
    ...
    system.main()
    file_name = pwd+"/../../data/pred"+str(system.PRED_HORIZON_STEP)+"_cntl"+str(system.CNTL_HORIZON_STEP)+"_mem"+str(system.MEMBER_NUM)+"_inf"+"{:.2f}".format(system.INFLATION)+"_u3_rs-mem-largest_sp0x15.csv"
